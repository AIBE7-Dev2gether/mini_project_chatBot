# ArChat Spring Boot + JPA + Aiven MySQL 마이그레이션 수행 기록

> 이 문서는 2026-07-24 완료된 마이그레이션의 설계·수행 기록입니다. 현재 설치 또는 운영 절차로 직접 사용하지 마세요. 최신 실행·배포 절차는 프로젝트 README와 `docs/DEPLOYMENT_AND_OPERATIONS.md`를 따릅니다.
...
- Render Docker Web Service 배포 및 공개 URL 응답 확인

운영 안정화 후속 항목:

- 운영용 Aiven CA 신원 검증(`VERIFY_CA` 또는 `VERIFY_IDENTITY`) 구성
- 사용자별 AI 요청 제한과 비용 알림 구성
- 롤백 보존 기간 종료 후 원본 DB 접속 환경 변수와 일회성 이관 도구 정리

현재 배포·운영 절차는 [Render · Aiven 배포 및 운영 가이드](../DEPLOYMENT_AND_OPERATIONS.md)를 따른다.
...
- `DatabaseUtil`
- PostgreSQL JDBC Driver 런타임 의존성(일회성 검증 테스트용 test scope는 안정화 기간 동안 유지)
...
- JDBC `DatabaseUtil`과 PostgreSQL runtime driver가 제거된다.
...
아래 내용은 마이그레이션을 시작할 당시의 첫 구현 PR 권고안이며 현재는 모두 완료됐다.
