...
import com.example.archat.application.auth.DefaultAuthUseCase;
...
public class AuthController extends BaseController {
    ...
    private final AuthUseCase authUseCase = DefaultAuthUseCase.getInstance();
    ...
    private void handleLoginPage(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            AuthUser authUser = authUseCase.login(loginPayload.email(), loginPayload.password());
    ...
    private void handleSignupPage(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            AuthUser authUser = authUseCase.signup(signupPayload.email(), signupPayload.password());
            sessionManager.login(req, authUser);
            resp.sendRedirect(req.getContextPath() + "/chat");
        } catch (AuthException e) {
