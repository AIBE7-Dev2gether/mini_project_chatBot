...
- 메시지 전송: `ChatController#doPost`가 사용자 메시지를 `Chat`으로 만들고 `AIChatService.save()`를 호출합니다. 서비스는 사용자 메시지를 저장하고 채팅 이력을 조회한 뒤 모델명에 따라 `GenAIChatProvider` 또는 `OpenAICompatibleProvider` 중 하나를 호출해 AI 응답도 저장합니다.
